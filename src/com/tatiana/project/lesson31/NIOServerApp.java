...
public class NIOServerApp {
    ...
    private class NioThread extends Thread {
        private ServerSocketChannel serverChannel; // слушает клиентов, следит за новыми подкл-иями
        private HashSet<SocketChannel> channels = new HashSet<>(); // список каналов
        private static final int BUF_SIZE = 1024;
        private ByteBuffer byteBuffer;
        private Selector selector; //объект, подписыв-ся на каналы
        ...
        private void init() throws IOException {
            ...
            selector = Selector.open(); // создали селектор
        ...
        private void chooseEventReaction() throws IOException {
            ...
            Set<SelectionKey> keys = selector.selectedKeys(); // можно удалять объекты в методе .selectedKeys()
            // Set<SelectionKey> keys = selector.keys(); // нельзя удалять объекты в методе .keys()
