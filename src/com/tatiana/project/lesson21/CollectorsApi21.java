...
public class CollectorsApi21 {
    public static void main(String[] args) {
        ...
        // classfier лписывает правила формир-ия ключа в мапе для эл-та потока
        // (vehicle) -> vehicle.getLevelOfWare()
        Map<Integer, List<Vehicle>> vehiclesByLevelOfWare = vehicles.stream()
                .collect(Collectors.groupingBy(vehicle -> vehicle.getLevelOfWare() //правило формирования ключей
                        // ,Collectors.toCollection(ArrayList::new) //здесь правило формирования списка
                ));
        ...
        Map<Integer, List<String>> map05 = vehicles.stream()
                .collect(Collectors.groupingBy(vehicle -> vehicle.getLevelOfWare(),
                        Collectors.filtering(vehicle -> vehicle.getLevelOfWare() > 5, //здесь второу аргумент -
                                // собираем только те, которые нам подходят, эта строка относится к парам(ключ-значение)
                                Collectors.mapping(vehicle -> vehicle.getNumber(), Collectors.toList()))));

        Map<Integer, List<String>> map06 = vehicles.stream()   //здесь тоже что и map05
