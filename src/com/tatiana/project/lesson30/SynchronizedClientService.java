...
public class SynchronizedClientService extends Thread {
    ...
    @Override
    public void run() {
//        Вариант №1 потоки будут выполнять запросы последовательно, т.е тут не будет многопоточности, всегда послед-но,
//        потому что колл-ия блокируется вся
...
//        }
        ...
        try {
            semaphore.acquire();   //поток забрал разрешение семафора
            reentrantLock.lock();   //поток забрал разрешение блокировщика
            token = tokens.removeLast();
        ...
        semaphore.release();    //отпускает разрешение семафора
    }
